	...
	CCreateContext* pContext)
{
	...
	int nMenuSize = nSizeX / 5;						// 6분의 1사이즈로
	if (!m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CTool3DView),
		CSize(nSizeX - nMenuSize, 0), pContext))
		return FALSE;
	if (!m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CMainFormView),
		CSize(nMenuSize, 0), pContext))
		return FALSE;

	g_pView = (CTool3DView*)m_wndSplitter.GetPane(0, 0);
	m_view2 = (CMainFormView*)m_wndSplitter.GetPane(0, 1);
	m_view2->SetGame(g_pView->GetGame());
...
BOOL CMainFrame::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	SetTitle(_TEXT("Borderlands's Tool"));
	return TRUE;
}
...
LRESULT CSplitterX::OnNcHitTest(CPoint point) {
	return HTNOWHERE;		//분할 경계 프레임에 마우스가 있음을 리턴
}
