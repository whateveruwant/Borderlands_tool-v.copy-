...
void cQuadTree::CreateChildTree()
{
	//중심위치
	//각코너의 포지션 센터 값
	m_CenterPos = (m_pTerrainVertices[m_Corners[0]].pos + m_pTerrainVertices[m_Corners[1]].pos +
		m_pTerrainVertices[m_Corners[2]].pos +
		m_pTerrainVertices[m_Corners[3]].pos) * 0.25f;

	//아무점 하나의 거리가 반지름이다.
	float prevLengthSq = 0.0f;
	for (int i = 0; i < 4; ++i) {
...
void cQuadTree::ChangeVertexInfo(POINT xz, cTerrain::LPTERRAINVERTEX pTerrainVertices)
{
	//중심위치
	//각코너의 포지션 센터 값
	m_CenterPos = (m_pTerrainVertices[m_Corners[0]].pos + m_pTerrainVertices[m_Corners[1]].pos +
	...
	m_Radius = sqrt(prevLengthSq);
	if((m_pTerrainVertices[m_Corners[CORNER_LT]].pos.x <= xz.x && xz.x <= m_pTerrainVertices[m_Corners[CORNER_RB]].pos.x) &&
		(m_pTerrainVertices[m_Corners[CORNER_LT]].pos.z >= xz.y && xz.y >= m_pTerrainVertices[m_Corners[CORNER_RB]].pos.z)){
		//자식이잇니?
		if ((m_Corners[CORNER_RT] - m_Corners[CORNER_LT]) > 1)
		{
			//자식 재귀
			for (int i = 0; i < 4; i++)
			{
				m_pChilds[i]->ChangeVertexInfo(xz, pTerrainVertices);
			}
		}
		//자식이 없는 맨마지막 노드니?
		else
		{
			m_pTerrainVertices = pTerrainVertices;
