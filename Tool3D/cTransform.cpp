...
void cTransform::SetAxises(D3DXVECTOR3 * axis)
...
void cTransform::DefaultControl( float timeDelta )
{
	...
	if( KEY_MGR->IsOnceDown( VK_LSHIFT ) ){
	...
	//우클릭을 할때만 Default Control 을 한다
		...
		//이동 해라
		//타겟벡터 
		D3DXVECTOR3 target = 10.0f * inputVector * maxSpeed;
...
void cTransform::DefaultControl2( float timeDelta )
{
	...
	if( KEY_MGR->IsStayDown(VK_LSHIFT) == false )
	{
		if( KEY_MGR->IsStayDown( 'A' ) )
