...
namespace MyUtil {
	...
	HRESULT CreateDevice() {
		...
		HRESULT result = d3d9->CreateDevice(D3DADAPTER_DEFAULT,
			D3DDEVTYPE_HAL,
			g_hWnd,
			vertexProcessing,
			&stD3DPP,
	...
	POINT GetMousePos()
	{
		...
		ScreenToClient(g_hWnd, &mousePos); //Screen 상의 위치를 Client 영영역 범위로 바꾼다.
	...
	void ComputeNormalPoint(D3DXVECTOR3 * pNorams, const D3DXVECTOR3 * pVertices, int verticesNum, int index, POINT targetXZ)
	{
		if (targetXZ.x == sqrt(verticesNum) - 1 || targetXZ.y == sqrt(verticesNum) - 1 || targetXZ.x == 0 || targetXZ.y == 0) return;
		
		//해당 삼각형의 정점 인덱스
		DWORD i0 = (targetXZ.y) * sqrt(verticesNum) + (targetXZ.x - 1);
		DWORD i1 = (targetXZ.y + 1) * sqrt(verticesNum) + (targetXZ.x);
		DWORD i2 = (targetXZ.y) * sqrt(verticesNum) + (targetXZ.x);
		...
		/////////////////////////////////////////////////////////////////////////////// 0
		//해당 삼각형의 정점 인덱스
		i0 = (targetXZ.y + 1) * sqrt(verticesNum) + (targetXZ.x);
		i1 = (targetXZ.y + 1) * sqrt(verticesNum) + (targetXZ.x + 1);
		i2 = (targetXZ.y) * sqrt(verticesNum) + (targetXZ.x);
		//정점 3개
		v0 = pVertices[i0];
		v1 = pVertices[i1];
		v2 = pVertices[i2];
		...
		/////////////////////////////////////////////////////////////////////////////// 1
		//해당 삼각형의 정점 인덱스
		i0 = (targetXZ.y) * sqrt(verticesNum) + (targetXZ.x);
		i1 = (targetXZ.y + 1) * sqrt(verticesNum) + (targetXZ.x + 1);
		i2 = (targetXZ.y) * sqrt(verticesNum) + (targetXZ.x + 1);
		//정점 3개
		v0 = pVertices[i0];
		v1 = pVertices[i1];
		v2 = pVertices[i2];
		...
		///////////////////////////////////////////////////////////////////////////////
		//해당 삼각형의 정점 인덱스
		i0 = (targetXZ.y) * sqrt(verticesNum) + (targetXZ.x - 1);
		i1 = (targetXZ.y) * sqrt(verticesNum) + (targetXZ.x);
		i2 = (targetXZ.y - 1) * sqrt(verticesNum) + (targetXZ.x - 1);
		//정점 3개
		v0 = pVertices[i0];
		v1 = pVertices[i1];
		v2 = pVertices[i2];
		...
		///////////////////////////////////////////////////////////////////////////////
		//해당 삼각형의 정점 인덱스
		i0 = (targetXZ.y - 1) * sqrt(verticesNum) + (targetXZ.x - 1);
		i1 = (targetXZ.y) * sqrt(verticesNum) + (targetXZ.x);
		i2 = (targetXZ.y - 1) * sqrt(verticesNum) + (targetXZ.x);
		//정점 3개
		v0 = pVertices[i0];
		v1 = pVertices[i1];
		v2 = pVertices[i2];
		...
		//해당 삼각형의 정점 인덱스
		i0 = (targetXZ.y) * sqrt(verticesNum) + (targetXZ.x);
		i1 = (targetXZ.y) * sqrt(verticesNum) + (targetXZ.x + 1);
		i2 = (targetXZ.y - 1) * sqrt(verticesNum) + (targetXZ.x);
		//정점 3개
		v0 = pVertices[i0];
		v1 = pVertices[i1];
		...
		D3DXVec3Normalize(&pNorams[index], &pNorams[index]);
	}

	void ComputeTangentAngBinormalPoint(D3DXVECTOR3 * outTangets, D3DXVECTOR3 * outBinormals, const D3DXVECTOR3 * pPositions, const D3DXVECTOR3 * pNormals, const D3DXVECTOR2 * pUVs, DWORD NumVertices, int index, POINT targetXZ)
	{
		//임시 Tangent Binormal 배열
		D3DXVECTOR3* pTangents = new D3DXVECTOR3[NumVertices];
		D3DXVECTOR3* pBinormals = new D3DXVECTOR3[NumVertices];
		...
		//해당 삼각형의 인덱스
		DWORD i0 = (targetXZ.y) * sqrt(NumVertices) + (targetXZ.x - 1);
		DWORD i1 = (targetXZ.y + 1) * sqrt(NumVertices) + (targetXZ.x);
		DWORD i2 = (targetXZ.y) * sqrt(NumVertices) + (targetXZ.x);
		//해당 삼각형의 정점위치
		D3DXVECTOR3 p0 = pPositions[i0];
		D3DXVECTOR3 p1 = pPositions[i1];
		D3DXVECTOR3 p2 = pPositions[i2];
		...
		//해당 삼각형의 인덱스
		i0 = (targetXZ.y + 1) * sqrt(NumVertices) + (targetXZ.x);
		i1 = (targetXZ.y + 1) * sqrt(NumVertices) + (targetXZ.x + 1);
		i2 = (targetXZ.y) * sqrt(NumVertices) + (targetXZ.x);
		//해당 삼각형의 정점위치
		p0 = pPositions[i0];
		p1 = pPositions[i1];
		p2 = pPositions[i2];
		...
		r =
			1.0f / ((uvEdge1.x * uvEdge2.y) - (uvEdge1.y * uvEdge2.x));
		//Tangent
		t = ((edge1 * uvEdge2.y) + (edge2 * -uvEdge1.y)) * r;
		D3DXVec3Normalize(&t, &t);
		//Binormal 
		b = ((edge1 * -uvEdge2.x) + (edge2 * uvEdge1.x)) * r;
		D3DXVec3Normalize(&b, &b);
		pTangents[i0] += t;
		pTangents[i1] += t;
		pTangents[i2] += t;
		...
		i0 = (targetXZ.y) * sqrt(NumVertices) + (targetXZ.x);
		i1 = (targetXZ.y + 1) * sqrt(NumVertices) + (targetXZ.x + 1);
		i2 = (targetXZ.y) * sqrt(NumVertices) + (targetXZ.x + 1);
		//해당 삼각형의 정점위치
		p0 = pPositions[i0];
		p1 = pPositions[i1];
		p2 = pPositions[i2];
		...
		i0 = (targetXZ.y) * sqrt(NumVertices) + (targetXZ.x - 1);
		i1 = (targetXZ.y) * sqrt(NumVertices) + (targetXZ.x);
		i2 = (targetXZ.y - 1) * sqrt(NumVertices) + (targetXZ.x - 1);
		//해당 삼각형의 정점위치
		p0 = pPositions[i0];
		p1 = pPositions[i1];
		p2 = pPositions[i2];
		...
		i0 = (targetXZ.y - 1) * sqrt(NumVertices) + (targetXZ.x - 1);
		i1 = (targetXZ.y) * sqrt(NumVertices) + (targetXZ.x);
		i2 = (targetXZ.y - 1) * sqrt(NumVertices) + (targetXZ.x);
		//해당 삼각형의 정점위치
		p0 = pPositions[i0];
		p1 = pPositions[i1];
		p2 = pPositions[i2];
		...
		i0 = (targetXZ.y) * sqrt(NumVertices) + (targetXZ.x);
		i1 = (targetXZ.y) * sqrt(NumVertices) + (targetXZ.x + 1);
		i2 = (targetXZ.y - 1) * sqrt(NumVertices) + (targetXZ.x);
		//해당 삼각형의 정점위치
		p0 = pPositions[i0];
		p1 = pPositions[i1];
		...
		D3DXVECTOR3 n = pNormals[index];
		//Tangent 그람슈미트 직교
		D3DXVECTOR3 t2 = pTangents[index] -
			(D3DXVec3Dot(&pTangents[index], &n) * n);
		D3DXVec3Normalize(&t2, &t2);
		//노말과 직교화된 T 와 외적하여 B
		D3DXVECTOR3 b2;
		D3DXVec3Cross(&b2, &n, &t);
		D3DXVec3Normalize(&b2, &b2);
		outTangets[index] = t2;
		outBinormals[index] = b2;

		SAFE_DELETE(pTangents);
		SAFE_DELETE(pBinormals);
	}				 
